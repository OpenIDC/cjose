...
// parameter. The CEK is then wrapped with AES Key Wrap. RFC 7518 requires a
// salt input of at least 8 octets and recommends at least 1000 iterations. On
// decrypt "p2c" is attacker-controlled, so a maximum bounds the PBKDF2 work a
// JWE can demand; it can be raised at build time.
...
static bool _cjose_jwe_encrypt_ek_pbes2(_jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, cjose_err *err)
{
    ...
    if (p2s_len < CJOSE_JWE_PBES2_MIN_SALT_LEN || p2c < CJOSE_JWE_PBES2_MIN_ITERATIONS || p2c > CJOSE_JWE_PBES2_MAX_ITERATIONS)
    {
        CJOSE_ERROR(err, CJOSE_ERR_INVALID_ARG);
...
static bool _cjose_jwe_decrypt_ek_pbes2(_jwe_int_recipient_t *recipient, cjose_jwe_t *jwe, const cjose_jwk_t *jwk, cjose_err *err)
{
    ...
    if (p2s_len < CJOSE_JWE_PBES2_MIN_SALT_LEN)
    {
        CJOSE_ERROR(err, CJOSE_ERR_INVALID_ARG);
